    ...
    <div className="max-w-5xl mx-auto pb-12 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500">Manage your profile, security, and support.</p>
        </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 flex flex-col items-center text-center relative overflow-hidden">
            ...
            <div className="w-full space-y-2">
              ...
              <div className="pt-4 mt-4 border-t border-gray-100"><button onClick={() => { if(window.confirm("Sign out?")) onLogout(); }} className="w-full flex items-center justify-between p-4 rounded-2xl bg-red-50 text-malawi-red"><div className="flex items-center gap-3"><LogOut size={18} /><span className="text-sm font-bold uppercase tracking-tight">Sign Out</span></div><ChevronRight size={14} /></button></div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-gray-100 min-h-[600px] flex flex-col">
            {activeTab === 'account' && (
              <form onSubmit={handleUpdateProfile} className="space-y-8 animate-in fade-in slide-in-from-right-4">
                <h3 className="text-xl font-black uppercase tracking-tight">Account Visualization</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Current Base</label><input type="text" className="w-full p-4 bg-gray-50 border rounded-2xl outline-none" value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})} /></div>
                  ...
                  <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">WhatsApp</label><input type="tel" className="w-full p-4 bg-gray-50 border rounded-2xl outline-none" value={formData.whatsapp} onChange={e => setFormData({...formData, whatsapp: e.target.value})} /></div>
                </div>
                <div className="space-y-3"><div className="flex justify-between items-center"><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Network Bio</label><button type="button" onClick={generateAIBio} disabled={isGeneratingBio} className="text-malawi-green text-[9px] font-black uppercase flex items-center gap-1 group">{isGeneratingBio ? <Loader2 size={12} className="animate-spin" /> : <Sparkles size={12} />} AI Bio</button></div><textarea rows={4} className="w-full p-5 bg-gray-50 border rounded-3xl outline-none resize-none" value={formData.bio} onChange={e => setFormData({...formData, bio: e.target.value})} /></div>
                <div className="pt-6 border-t border-gray-100"><h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Notification Preferences</h4><div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><PreferenceToggle label="Email: Financial" isActive={notifPrefs.emailWithdrawal} onToggle={() => setNotifPrefs({...notifPrefs, emailWithdrawal: !notifPrefs.emailWithdrawal})} icon={Mail} /><PreferenceToggle label="WhatsApp: Growth" isActive={notifPrefs.whatsappReferral} onToggle={() => setNotifPrefs({...notifPrefs, whatsappReferral: !notifPrefs.whatsappReferral})} icon={Smartphone} /></div></div>
            ...
            {activeTab === 'security' && (
              <form onSubmit={handleUpdateSecurity} className="space-y-8 animate-in fade-in slide-in-from-right-4">
                <h3 className="text-xl font-black uppercase tracking-tight">Security Center</h3>
                {securityError && <div className="p-4 bg-red-50 text-red-600 rounded-2xl text-[10px] font-black uppercase border border-red-100"><AlertCircle size={18} className="inline mr-2" /> {securityError}</div>}
                <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Username (ID)</label><div className="relative"><AtSign className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" size={18} /><input type="text" className="w-full pl-12 pr-4 py-4 bg-gray-50 border rounded-2xl outline-none" value={formData.username} onChange={e => setFormData({...formData, username: e.target.value})} /></div></div>
                <div className="space-y-4"><input type="password" placeholder="Current Password" className="w-full p-4 bg-gray-50 border rounded-2xl outline-none" value={formData.currentPassword} onChange={e => setFormData({...formData, currentPassword: e.target.value})} /><div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><input type="password" placeholder="New Password" className="w-full p-4 bg-gray-50 border rounded-2xl outline-none" value={formData.newPassword} onChange={e => setFormData({...formData, newPassword: e.target.value})} /><input type="password" placeholder="Verify New" className="w-full p-4 bg-gray-50 border rounded-2xl outline-none" value={formData.confirmPassword} onChange={e => setFormData({...formData, confirmPassword: e.target.value})} /></div></div>
                <button type="submit" disabled={isUpdatingSecurity} className="w-full bg-malawi-red text-white font-black py-5 rounded-2xl uppercase tracking-widest text-xs">Update Security</button>
              </form>
            )}
            ...
            {activeTab === 'support' && (
              <div className="space-y-6 animate-in fade-in h-full flex flex-col">
                <div className="flex items-center justify-between border-b pb-6">
                  ...
                  {!showSupportForm && !selectedTicketId && <button onClick={() => setShowSupportForm(true)} className="bg-malawi-green text-white px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest flex items-center gap-2 shadow-lg shadow-green-500/20 active:scale-95 transition-all"><PlusCircle size={14} /> New Ticket</button>}
                </div>
                ...
                {showSupportForm ? (
                  <form onSubmit={handleSupportSubmit} className="space-y-4 animate-in zoom-in-95">
                    ...
                    {/* Fix: Correct setter function names to match support state definitions */}
                    ...
                    <button type="submit" disabled={isSubmittingSupport} className="w-full bg-malawi-black text-white font-black py-4 rounded-2xl flex items-center justify-center gap-3 uppercase text-xs">
                      {isSubmittingSupport ? <Loader2 size={18} className="animate-spin" /> : <Send size={18} />} Send to Team
                    </button>
                  </form>
                ) : selectedTicket ? (
                  <div className="space-y-8 animate-in slide-in-from-right-4"><button onClick={() => setSelectedTicketId(null)} className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-400"><ArrowLeft size={16} /> Back to Inbox</button><div className="bg-white border rounded-3xl p-8 shadow-sm"><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Ticket: {new Date(selectedTicket.createdAt).toLocaleDateString()}</p><h4 className="text-2xl font-black mb-6">{selectedTicket.subject}</h4><div className="bg-gray-50 p-6 rounded-2xl text-sm italic text-gray-600 border">"{selectedTicket.message}"</div></div>{selectedTicket.reply ? <div className="flex gap-4 items-start"><div className="w-12 h-12 rounded-2xl bg-malawi-black flex items-center justify-center shrink-0 border-b-4 border-malawi-red"><ShieldCheck size={24} className="text-malawi-red" /></div><div className="bg-malawi-black text-white p-6 rounded-[2rem] rounded-tl-none flex-grow border-b-8 border-malawi-red/20"><div className="flex items-center gap-2 mb-2"><p className="text-[10px] font-black uppercase tracking-[0.2em] text-malawi-red">Support Verdict</p></div><p className="text-sm leading-relaxed whitespace-pre-wrap">{selectedTicket.reply}</p></div></div> : <div className="flex items-center gap-4 p-6 bg-yellow-50 rounded-2xl border text-yellow-700"><Clock size={24} className="animate-pulse" /><div><p className="text-xs font-black uppercase tracking-widest">In Progress</p><p className="text-[10px]">Reviewing your request.</p></div></div>}</div>
                ) : (
                  <div className="flex-grow space-y-3 overflow-y-auto max-h-[600px]">{myComplaints.length === 0 ? <div className="text-center p-12 text-gray-400">Box Empty.</div> : myComplaints.map(ticket => <button key={ticket.id} onClick={() => setSelectedTicketId(ticket.id)} className="w-full bg-white rounded-3xl p-6 border hover:border-malawi-black flex items-center justify-between group"><div className="flex items-center gap-4"><div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${ticket.status === 'PENDING' ? 'bg-yellow-50 text-yellow-600' : 'bg-green-50 text-green-600'}`}>{ticket.status === 'PENDING' ? <Clock size={20} /> : <CheckCircle size={20} />}</div><div><h5 className="font-black text-sm uppercase tracking-tight">{ticket.subject}</h5><span className="text-[9px] text-gray-400 font-bold uppercase">{new Date(ticket.createdAt).toLocaleDateString()}</span></div></div><ChevronRight size={18} className="text-gray-300 group-hover:text-malawi-black" /></button>)}</div>
                )}
              </div>
            )}
          </div>
