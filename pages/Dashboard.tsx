  ...
  return (
    <div className="space-y-6 pb-24 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Direct Commission</p>
              <p className="text-xl font-black text-malawi-green">{currentTier?.directCommission || 0}% Attached</p>
            </div>
          </div>
        </div>
        <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Indirect Commission</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white p-6 rounded-3xl border shadow-sm text-center">
              <p className="text-[10px] font-black uppercase text-gray-400">Total Profits</p>
              <p className="text-xl font-black text-malawi-green">K{user.totalEarnings.toLocaleString()}</p>
            </div>
            <div className="bg-white p-6 rounded-3xl border shadow-sm text-center">
              <p className="text-[10px] font-black uppercase text-gray-400">Invite Count</p>
          ...
          <div className="bg-malawi-green p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden">
            <div className="relative z-10 space-y-4">
              <h2 className="text-2xl font-black uppercase tracking-tight">Recruit New Members</h2>
          ...
          <div className="bg-white rounded-[2.5rem] border shadow-sm p-8 space-y-4">
            <h3 className="text-sm font-black uppercase tracking-widest flex items-center gap-2"><Sparkles size={16} className="text-malawi-red"/> Marketing Assistant</h3>
            ...
            <form onSubmit={askMarketingAssistant} className="flex gap-2">
              <input value={aiPrompt} onChange={e => setAiPrompt(e.target.value)} placeholder="Ask for recruiting tips..." className="flex-grow bg-gray-50 p-4 rounded-2xl border outline-none text-sm"/>
        ...
        {/* INVITED PEOPLE LIST - WITH FULL LEGAL NAMES */}
        <div className="lg:col-span-1">
          <div className="bg-white rounded-[2.5rem] border shadow-sm h-full flex flex-col overflow-hidden max-h-[600px]">
            <div className="p-6 border-b bg-gray-50/50 flex justify-between items-center">
               ...
               <Award size={16} className="text-malawi-green" />
            </div>
            <div className="flex-grow overflow-y-auto divide-y">
              {myReferrals.length === 0 ? (
                <div className="p-12 text-center text-gray-300 italic text-xs uppercase font-black">No network data found</div>
              ) : (
                myReferrals.map(ref => {
                  ...
                  return (
                    <div key={ref.id} className="p-5 hover:bg-gray-50 transition-colors">
                       <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-malawi-black rounded-xl flex items-center justify-center text-white font-black uppercase">{target?.fullName.charAt(0)}</div>
                          <div className="flex-grow">
                             ...
                             <div className="flex items-center gap-2 mt-0.5">
                                <span className={`text-[8px] font-black px-2 py-0.5 rounded uppercase ${target?.membershipStatus === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                                  {target?.membershipTier || 'FREE'}
                                </span>
                                <span className="text-[9px] font-bold text-gray-400">Joined {new Date(ref.timestamp).toLocaleDateString()}</span>
                             </div>
                          </div>
                          <div className="text-right">
                             <p className="text-[10px] font-black text-malawi-green">+K{ref.commission.toLocaleString()}</p>
                             <p className="text-[8px] font-bold text-gray-300 uppercase">L{ref.level}</p>
                          </div>
                       </div>
                    </div>
                  );
