...
import { 
  ...
  Smartphone, ExternalLink, MessageSquareWarning, CreditCard
} from 'lucide-react';
        ...
        {tab === 'withdrawals' && (
          <div className="p-0">
             <div className="p-8 border-b bg-gray-50/50 flex justify-between items-center">
                ...
                <span className="text-[10px] font-black bg-white border px-3 py-1 rounded-full uppercase tracking-widest text-gray-400">Identity Verified Mode</span>
             </div>
               ...
               <table className="w-full text-left">
                 <thead className="bg-gray-50 text-[10px] font-black uppercase text-gray-400 border-b">
                   <tr>
                     ...
                     <th className="px-10 py-6">Method & Amount</th>
                     <th className="px-10 py-6">Contact & Status</th>
                     <th className="px-10 py-6 text-center">Final Action</th>
                   </tr>
                 </thead>
                 <tbody className="divide-y">
                   {filteredWithdrawals.length === 0 ? (
                     ...
                     <tr key={w.id} className={`hover:bg-gray-50/50 transition-colors ${w.status === 'PENDING' ? 'bg-yellow-50/5' : ''}`}>
                       ...
                       <td className="px-10 py-8">
                         {w.proofUrl ? (
                           <button onClick={() => viewImage(w.proofUrl)} className="group relative w-32 h-20 rounded-xl overflow-hidden border-2 border-white shadow-lg transition-transform hover:scale-110">
                              <img src={w.proofUrl} className="w-full h-full object-cover" />
                              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center text-white"><Eye size={16} /></div>
                           </button>
                         ) : <span className="text-[9px] font-black text-gray-300 uppercase italic">No Document</span>}
                       </td>
                       <td className="px-10 py-8">
                       ...
                       <td className="px-10 py-8">
                         <div className="flex items-center gap-2 mb-3">
                            <a href={`tel:${w.phone}`} className="p-2 bg-gray-100 rounded-lg hover:bg-malawi-green hover:text-white transition-all"><Phone size={14} /></a>
                            <a href={`https://wa.me/${w.whatsapp}`} target="_blank" className="p-2 bg-gray-100 rounded-lg hover:bg-[#25D366] hover:text-white transition-all"><MessageCircle size={14} /></a>
                            <span className={`ml-2 px-3 py-1 rounded-full text-[9px] font-black uppercase ${w.status === 'PENDING' ? 'bg-yellow-100 text-yellow-700' : w.status === 'APPROVED' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                               {w.status}
                            </span>
                         </div>
                         {w.adminNote && <p className="text-[10px] text-gray-400 italic max-w-xs truncate">"{w.adminNote}"</p>}
                       </td>
                       <td className="px-10 py-8">
                         {w.status === 'PENDING' ? (
                           <div className="flex flex-col gap-2">
                             ...
                             <div className="flex gap-2">
                               <button onClick={() => handleWithdrawalAction(w.id, WithdrawalStatus.APPROVED)} className="flex-grow bg-malawi-green text-white py-2 rounded-lg text-[9px] font-black uppercase shadow-sm">Pay</button>
                               <button onClick={() => handleWithdrawalAction(w.id, WithdrawalStatus.REJECTED)} className="flex-grow bg-malawi-red text-white py-2 rounded-lg text-[9px] font-black uppercase shadow-sm">Refuse</button>
                             </div>
                           </div>
                         ) : (
                           <div className="text-center opacity-50">
                             <CheckCircle2 size={24} className="mx-auto text-gray-300" />
