...
const Dashboard: React.FC<DashboardProps> = ({ state, onStateUpdate }) => {
  ...
  // Notifications logic for mutual interaction
  const notifications = useMemo(() => {
    ...
    state.withdrawals.filter(w => w.userId === user.id && w.status !== WithdrawalStatus.PENDING).forEach(w => {
      list.push({
        ...
        body: w.adminNote || (w.status === WithdrawalStatus.APPROVED ? 'Admin has sent your payment confirmation receipt.' : 'Payout request was rejected by administration.'),
        date: w.createdAt,
    ...
    state.complaints.filter(c => c.userId === user.id && c.reply).forEach(c => {
      list.push({
        ...
        body: `New response regarding: ${c.subject}`,
        date: c.updatedAt,
        icon: MessageSquare,
  ...
  const askMarketingAssistant = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `You are an expert affiliate mentor for the 'KPH TRANSFORM MALAWI' platform. Provide actionable growth advice for: "${aiPrompt}".`,
      });
      setAiResponse(response.text);
    } catch (err) {
      setAiResponse("Network busy. Please try again.");
    } finally {
      setIsAskingAI(false);
    }
  };
  ...
  return (
    <div className="space-y-6 pb-24 animate-in fade-in duration-700">
      ...
      {/* MUTUAL INTERACTION: Prominent Global Notification Bar */}
      {latestAlert && (
        <div className={`p-1 rounded-[2.5rem] shadow-xl animate-in slide-in-from-top-4 duration-500 ring-4 ${latestAlert.type === 'success' ? 'bg-malawi-green ring-malawi-green/10' : 'bg-malawi-red ring-malawi-red/10'}`}>
           <div className="bg-white px-8 py-4 rounded-[2.2rem] flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                 ...
                 <div>
                   <p className="text-[10px] font-black uppercase text-gray-400 tracking-widest">System Update</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm flex items-center gap-4 group hover:border-malawi-green transition-all">
          ...
          <div>
            <p className="text-[10px] font-black uppercase text-gray-400 tracking-widest">DIRECT EARNINGS (30%-40%)</p>
        ...
        <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm flex items-center gap-4 group hover:border-malawi-red transition-all">
          ...
          <div>
            <p className="text-[10px] font-black uppercase text-gray-400 tracking-widest">NETWORK BONUS (5%)</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-malawi-green p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden">
            ...
            <p className="text-[10px] font-bold uppercase text-white/60 relative z-10 mb-4">Start inviting your network and earn massive commissions.</p>
        ...
        <div className="lg:col-span-4 space-y-6">
           <div className="bg-white rounded-[2.5rem] border shadow-sm p-8 space-y-4 h-full">
             <h3 className="text-sm font-black uppercase tracking-widest flex items-center gap-2 text-malawi-red"><Sparkles size={16}/> Business Mentor</h3>
             <div className="p-4 bg-gray-50 rounded-2xl text-[11px] font-medium border italic min-h-[100px] leading-relaxed">
               {aiResponse ? `"${aiResponse}"` : "Ask your AI assistant for advice on how to grow your Malawian affiliate network."}
             </div>
             <form onSubmit={askMarketingAssistant} className="flex gap-2">
               <input 
                 value={aiPrompt} 
                 onChange={e => setAiPrompt(e.target.value)} 
                 placeholder="Recruitment tips?" 
