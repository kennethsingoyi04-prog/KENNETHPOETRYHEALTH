...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ state, onStateUpdate }) => {
  const [tab, setTab] = useState<'withdrawals' | 'activations' | 'books' | 'complaints' | 'users'>('withdrawals');
  ...
  const handleProcessPayout = (status: WithdrawalStatus) => {
    ...
    if (status === WithdrawalStatus.APPROVED && (!payoutProofUrl || !payoutNote.trim())) {
      alert("Please provide the transaction ID and upload the payment receipt screenshot.");
      return;
    }
  ...
  const handleActivationAction = (targetId: string, status: 'APPROVE' | 'REJECT') => {
    const targetUser = state.users.find(u => u.id === targetId);
    if (!targetUser) return;

    if (status === 'REJECT') {
      const updatedUsers = state.users.map(u => u.id === targetId ? { 
        ...u, 
        membershipStatus: MembershipStatus.INACTIVE, 
        membershipNote: "Your activation request was rejected. Please ensure you uploaded the correct receipt and try again." 
      } : u);
      onStateUpdate({ users: updatedUsers });
      alert("Activation request rejected. User has been informed.");
      return;
    }
    ...
    updatedUsers = updatedUsers.map(u => u.id === targetId ? { 
      ...u, 
      membershipStatus: MembershipStatus.ACTIVE, 
      membershipNote: "Account fully activated. Welcome to the KPH network!" 
    } : u);
    ...
    alert("Activation confirmed and profits distributed.");
  };
  ...
  const handleBookSellerAction = (userId: string, status: BookSellerStatus) => {
    const updatedUsers = state.users.map(u => u.id === userId ? { 
      ...
      membershipNote: status === BookSellerStatus.APPROVED ? "Your Distributor status is now active." : "Your Distributor application was rejected."
    } : u);
    onStateUpdate({ users: updatedUsers });
    alert(`Distributor application status updated.`);
  };
  ...
  return (
    <div className="max-w-7xl mx-auto pb-32 animate-in fade-in duration-700">
      {viewingProofUrl && (
        <div className="fixed inset-0 z-[300] bg-black/95 flex items-center justify-center p-8" onClick={() => setViewingProofUrl(null)}>
        ...
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-malawi-black/90 backdrop-blur-md">
           <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden">
              <div className="bg-malawi-black p-8 text-white flex justify-between items-center">
                 <h2 className="text-xl font-black uppercase tracking-tight">Support Reply</h2>
                 <button onClick={() => setReplyingTo(null)} className="p-2 hover:bg-white/10 rounded-full"><X size={24}/></button>
              </div>
              <div className="p-8 space-y-6">
                 <div className="bg-gray-50 p-6 rounded-2xl border">
                    <p className="text-[10px] font-black text-gray-400 uppercase mb-2">Issue Ticket</p>
                 ...
                 <form onSubmit={handleSupportReply} className="space-y-4">
                    <textarea required rows={4} className="w-full p-5 bg-gray-50 border rounded-2xl text-sm outline-none font-bold" placeholder="Write your reply..." value={adminReplyText} onChange={e => setAdminReplyText(e.target.value)} />
                    <button type="submit" className="w-full py-5 bg-malawi-green text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg flex items-center justify-center gap-2 transition-all">
      ...
      {processingPayout && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-malawi-black/90 backdrop-blur-md">
           <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden">
              <div className="bg-malawi-black p-8 text-white flex justify-between items-center">
                 <div>
                    <h2 className="text-xl font-black uppercase tracking-tight">Confirm Withdrawal</h2>
              ...
              <div className="p-8 space-y-6">
                 <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-gray-400 ml-1">Screenshot Reference</label>
                 ...
                 <div className="grid grid-cols-2 gap-3">
                    <button onClick={() => handleProcessPayout(WithdrawalStatus.APPROVED)} className="py-4 bg-malawi-green text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl transition-all">Confirm</button>
                    <button onClick={() => handleProcessPayout(WithdrawalStatus.REJECTED)} className="py-4 bg-malawi-red text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl transition-all">Reject</button>
                 </div>
              </div>
           </div>
        </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-malawi-black/95 backdrop-blur-xl">
           <div className="bg-white w-full max-w-4xl rounded-[4rem] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
              <div className="p-8 bg-malawi-black text-white flex items-center justify-between">
                 <h2 className="text-2xl font-black uppercase tracking-tight">Affiliate Analysis: {inspectingUser.fullName}</h2>
                 ...
                 <div className="bg-white border rounded-[2rem] overflow-hidden shadow-inner">
                    <table className="w-full text-left">
                       <thead className="bg-gray-50 text-[10px] font-black uppercase text-gray-400 border-b">
                          <tr><th className="px-8 py-4">Referred Member</th><th className="px-8 py-4">Level</th><th className="px-8 py-4 text-right">Commission</th></tr>
                       </thead>
                       <tbody className="divide-y text-xs">
                          {inspectingUserNetwork.map(r => (
                             <tr key={r.id}>
                                <td className="px-8 py-5 font-black uppercase">{r.user?.fullName}</td>
                                <td className="px-8 py-5 font-bold text-gray-400 uppercase">{r.level === 1 ? 'Direct' : 'Indirect'}</td>
      ...
      <header className="mb-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="flex items-center gap-5">
          ...
          <h1 className="text-4xl font-black uppercase tracking-tight">KPH Admin</h1>
        </div>
      ...
      <div className="flex gap-3 overflow-x-auto pb-6 scrollbar-hide">
        {[
          { id: 'withdrawals', label: 'Withdrawals', icon: Wallet, count: pendingWithdrawals.length },
          { id: 'activations', label: 'Activation', icon: ShieldAlert, count: pendingActivations.length },
          { id: 'books', label: 'Books', icon: BookOpen, count: pendingBookSellers.length },
      ...
      <main className="bg-white rounded-[4rem] border shadow-2xl overflow-hidden min-h-[600px]">
        {tab === 'withdrawals' && (
           <table className="w-full text-left">
              <thead className="bg-gray-50 text-[10px] font-black uppercase text-gray-400 border-b">
                 <tr><th className="px-10 py-6">Affiliate</th><th className="px-10 py-6">Requested</th><th className="px-10 py-6">Method</th><th className="px-10 py-6">Contact</th><th className="px-10 py-6 text-right">Action</th></tr>
              </thead>
              <tbody className="divide-y">
                 {pendingWithdrawals.length === 0 ? <tr><td colSpan={5} className="p-20 text-center text-gray-300 font-black uppercase">No pending withdrawals</td></tr> :
                   pendingWithdrawals.map(w => (
                      <tr key={w.id} className="hover:bg-gray-50">
                         ...
                         <td className="px-10 py-8 font-bold text-gray-400 uppercase">{w.paymentMethod}</td>
                         <td className="px-10 py-8 font-bold text-gray-600">{w.phone}</td>
                         <td className="px-10 py-8 text-right"><button onClick={() => setProcessingPayout(w)} className="px-8 py-3 bg-malawi-black text-white rounded-2xl text-[10px] font-black uppercase shadow-lg active:scale-95 transition-all">Verify & Pay</button></td>
                      </tr>
                   ))
                 }
              </tbody>
        ...
        {tab === 'activations' && (
           <div className="divide-y">
              {pendingActivations.length === 0 ? <div className="p-20 text-center text-gray-300 font-black uppercase">No pending activations</div> : 
                    ...
                    <div key={u.id} className="p-10 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:bg-gray-50">
                       <div className="flex items-center gap-6">
                          ...
                          <div>
                             ...
                             <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1">Tier: <span className="text-malawi-red">{u.membershipTier}</span></p>
                          </div>
                       </div>
                       <div className="flex items-center gap-3">
                          ...
                          <button onClick={() => handleActivationAction(u.id, 'APPROVE')} className="px-6 py-4 bg-malawi-green text-white rounded-xl font-black uppercase text-[10px] tracking-widest shadow-xl transition-all active:scale-95">Approve</button>
                          <button onClick={() => handleActivationAction(u.id, 'REJECT')} className="px-6 py-4 bg-malawi-red text-white rounded-xl font-black uppercase text-[10px] tracking-widest shadow-xl transition-all active:scale-95">Reject</button>
                       </div>
                    </div>
                 ))
              }
        ...
        {tab === 'books' && (
           <div className="p-10 space-y-8">
              {pendingBookSellers.length === 0 ? <div className="p-20 text-center text-gray-300 font-black uppercase">No pending applications</div> : 
                    ...
                    <div key={u.id} className="bg-gray-50 rounded-[3rem] p-10 border border-gray-100 shadow-sm hover:shadow-md transition-all">
                       <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-10">
                          <div className="space-y-4">
                             <div className="flex items-center gap-4">
                                ...
                                <div>
                                   ...
                                   <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Book Distributor Portal</p>
                                </div>
                             </div>
                          ...
                          <div className="flex-grow max-w-md bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm">
                             <p className="text-[9px] font-black uppercase text-gray-400 mb-2 flex items-center gap-2"><MapPin size={12}/> Address Verified locally</p>
                             <p className="text-xs font-bold text-gray-700 leading-relaxed italic">"{u.bookSellerAddress || 'No address provided'}"</p>
                          </div>
                          <div className="flex flex-col gap-3 shrink-0">
                             <button onClick={() => handleBookSellerAction(u.id, BookSellerStatus.APPROVED)} className="px-10 py-4 bg-malawi-green text-white rounded-xl font-black uppercase text-[10px] tracking-widest shadow-xl active:scale-95 transition-all">Approve</button>
        ...
        {tab === 'complaints' && (
           <div className="divide-y">
              {pendingComplaints.length === 0 ? <div className="p-20 text-center text-gray-300 font-black uppercase">No pending complaints</div> : 
                 pendingComplaints.map(c => (
                    <div key={c.id} className="p-10 flex items-center justify-between hover:bg-gray-50">
                       <div className="space-y-1">
                          ...
                          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Affiliate: {c.userName}</p>
                          <p className="text-xs text-gray-600 mt-2 italic">"{c.message}"</p>
                       </div>
                       <button onClick={() => setReplyingTo(c)} className="px-8 py-3 bg-malawi-black text-white rounded-2xl text-[10px] font-black uppercase shadow-lg flex items-center gap-2 transition-all"><Reply size={16}/> Answer</button>
                    </div>
                 ))
              }
           </div>
        ...
        {tab === 'users' && (
           <div className="overflow-x-auto">
              <table className="w-full text-left">
                 <thead className="bg-gray-50 text-[10px] font-black uppercase text-gray-400 border-b">
                    <tr><th className="px-10 py-6">Affiliate</th><th className="px-10 py-6">Tier</th><th className="px-10 py-6">Balance</th><th className="px-10 py-6 text-right">Analysis</th></tr>
                 </thead>
                 <tbody className="divide-y">
                    {filteredUsers.map(u => (
                       <tr key={u.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-10 py-8 text-right"><button onClick={() => setInspectingUser(u)} className="px-8 py-3 bg-malawi-black text-white rounded-2xl text-[10px] font-black uppercase shadow-md active:scale-95">Inspect</button></td>
                       </tr>
                    ))}
                 </tbody>
              </table>
